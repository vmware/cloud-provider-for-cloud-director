...
func (cgm *CpiGatewayManager) UpdateLoadBalancer(ctx context.Context, lbPoolName string, virtualServiceName string,
	ips []string, internalPort int32, externalPort int32, oneArm *vcdsdk.OneArm, useVsSharedIP bool) error {
	...
	if err := gm.UpdateLoadBalancer(ctx, lbPoolName, virtualServiceName, ips, internalPort, externalPort, oneArm, useVsSharedIP); err != nil {
		return fmt.Errorf(
