...
func (a *VmcSddcsApiService) GetVmcSddcs(ctx context.Context, serviceAppId string) (VmcSddcs, *http.Response, error) {
	var (
		...
		localVarReturnValue VmcSddcs
	)

	// create path and map variables
	localVarPath := a.client.cfg.BasePath + "/1.0.0/serviceApps/{serviceAppId}/sddcs"
	localVarPath = strings.Replace(localVarPath, "{"+"serviceAppId"+"}", fmt.Sprintf("%v", serviceAppId), -1)
...
func (a *VmcSddcsApiService) RegisterVmcSDDC(ctx context.Context, serviceAppSddc VmcSddc, serviceAppId string) (*http.Response, error) {
	...
	// body params
	localVarPostBody = &serviceAppSddc
	if ctx != nil {
		// API Key Authentication
		if auth, ok := ctx.Value(ContextAPIKey).(APIKey); ok {
			var key string
